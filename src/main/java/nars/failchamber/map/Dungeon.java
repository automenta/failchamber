package nars.failchamber.map;

import nars.failchamber.state.Cell;
import nars.failchamber.state.Cell.Material;
...
public class Dungeon {
    ...
    public static int GetFeatureLowerBound(int c, int len) {
        ...
        /*
        public boolean IsWall(int x, int y, int xlen, int ylen, int xt, int yt, int d)        {
    ...
    public static int GetRand(int min, int max) {
        ...
        /*
        public static List<PointI> GetRoomPoints(int x, int y, int xlen, int ylen, int d)
        {
    ...
    public boolean MakeCorridor(int x, int y, int length, int direction) {
        ...
        int len = Dungeon.GetRand(2, length);
        ...
        switch (direction) {
            ...
            case Hauto.LEFT:
                // west
                if (ytemp > this._ysize) return false;
                ytemp = y;
        ...
        public IEnumerable<Tuple<PointI, int>> GetSurroundingPoints(PointI v)
        {
            var points = new[]
                             {
                                 Tuple.Create(new PointI { X = v.X, Y = v.Y + 1 }, Hauto.UP),
                                 Tuple.Create(new PointI { X = v.X - 1, Y = v.Y }, Hauto.RIGHT),
                                 Tuple.Create(new PointI { X = v.X , Y = v.Y-1 }, Hauto.DOWN),
                                 Tuple.Create(new PointI { X = v.X +1, Y = v.Y  }, Hauto.LEFT),
